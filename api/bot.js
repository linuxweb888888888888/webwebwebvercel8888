...
function fetchGlobalMarketsPromise() {
    return new Promise(async (resolve) => {
        try {
            console.log("🔍 Fetching Global Contract Sizes from HTX...");
            const markets = await htxOracle.fetchMarkets();
            if(markets && markets.length > 0) {
                markets.forEach(m => {
                    if (m.symbol && m.contractSize) global.marketSizes[m.symbol] = m.contractSize;
                });
                console.log(`✅ Global Contract Sizes Synced! (${Object.keys(global.marketSizes).length} markets)`);
            }
        } catch(e) { console.error("Market Size Fetch Error:", e.message); }
        resolve();
    });
}
...
function startPriceOracle() {
    ...
    }, 3600000); // Resync sizes every hour
}
                ...
                try {
                    ...
                    if (isTakeProfit || isStopLoss) {
                        ...
                        try {
                            ...
                            // FIXED: Longer Lock to prevent duplicate closing logic
                            cState.lockUntil = Date.now() + 15000;
                            cState.dcaCount = 0; 
                    ...
                    // FIXED: Exponential scaling to prevent instantaneous runaway grids!
                        ...
                        try {
                            if (!cw.isPaper) {
                                ...
                                await cw.exchange.createOrder(cw.symbol, 'market', closeSide, cw.contracts, undefined, { 
                                    offset: 'close', reduceOnly: true, lever_rate: cw.actualLeverage 
                                });
                            } else {
                                cw.cState.contracts = 0; cw.cState.unrealizedPnl = 0; cw.cState.currentRoi = 0; cw.cState.avgEntry = 0; cw.cState.dcaCount = 0;
                            }
                            
                            // FIXED: 60 Second Safety Lock to prevent DB duplicate reads
                            cw.cState.lockUntil = Date.now() + 60000;
...
const executeGlobalProfitMonitor = async () => {
    ...
    try {
        ...
        for (let userSetting of allUsersSettings) {
            ...
            // ==============================================================
            // ✨ NEW: EQUITY AUTO-BALANCER (TARGET NET UNREALIZED LIMIT)
            // ==============================================================
            if (userSetting.autoBalanceEquity && !offsetExecuted) {
                ...
                const balanceTolerance = 5.0 * multiplier; // Buffer to prevent dust loops

                if (globalUnrealized > targetPnl + balanceTolerance) {
                    // EXCEEDING TARGET (Less negative) -> Harvest winners to return Unrealized back down to target
                    ...
                    for (let w of winners) {
                        ...
                        try {
                            ...
                            if (bData) {
                                ...
                                OffsetModel.create({ userId: dbUserId, symbol: w.symbol, winnerSymbol: w.symbol, reason: 'Auto-Balance Harvest (Re-opened)', netProfit: realizedFromThis }).catch(()=>{});

                                logForProfile(firstProfileId, `⚖️ AUTO-BALANCE HARVEST: Closed & Re-opened ${closeQty} contracts of ${w.symbol} to secure +$${realizedFromThis.toFixed(4)} and lock matrix.`);
                                excess -= realizedFromThis;
                                offsetExecuted = true; 
                            }
                        } catch (e) {
                            logForProfile(firstProfileId, `❌ AUTO-BALANCE ERROR [${w.symbol}]: ${e.message}`);
                        }
                    }
                } else if (globalUnrealized < targetPnl - balanceTolerance) {
                    // BELOW TARGET (More negative) -> Burn losers using Realized PNL to pull Unrealized back up to target
                        ...
                        for (let l of losers) {
                            ...
                            try {
                                ...
                                if (bData) {
                                    ...
                                    OffsetModel.create({ userId: dbUserId, symbol: l.symbol, winnerSymbol: l.symbol, reason: 'Auto-Balance Forgive (Re-opened)', netProfit: lossRealized }).catch(()=>{});

                                    logForProfile(firstProfileId, `⚖️ AUTO-BALANCE FORGIVE: Burned -$${Math.abs(lossRealized).toFixed(4)} Realized PNL to reset ${closeQty} contracts of ${l.symbol} and lock matrix.`);
                                    budget -= Math.abs(lossRealized);
                                    offsetExecuted = true;
                                }
                            } catch (e) {
                                logForProfile(firstProfileId, `❌ AUTO-BALANCE ERROR [${l.symbol}]: ${e.message}`);
                            }
                        }
                    }
                }
            ...
            // SMART OFFSET V1
            if (!offsetExecuted && (smartOffsetNetProfit > 0 || smartOffsetBottomRowV1StopLoss < 0 || smartOffsetStopLoss < 0 || noPeakSlTimeframeSeconds > 0) && activeCandidates.length >= 2) {
                        ...
                        for (let k = 0; k < finalPairsToClose.length; k++) {
                            ...
                            try {
                                if (bData) {
                                    ...
                                    // FIXED: 60 Second Lock preventing DB Duplication
                                    if (bState) { bState.lockUntil = Date.now() + 60000; bState.dcaCount = 0; }
                                    ...
                                    if (bData) {
                                        if (!biggestWinner.isPaper) {
                                            ...
                                            await bData.exchange.createOrder(biggestWinner.symbol, 'market', closeSide, biggestWinner.contracts, undefined, { 
                                                offset: 'close', reduceOnly: true, lever_rate: biggestWinner.actualLeverage 
                                            });
                                        } else {
                                        ...
                                        // FIXED: 60 Second Lock
                                        if (bStateW) { bStateW.lockUntil = Date.now() + 60000; bStateW.dcaCount = 0; }
                        ...
                        for (let pos of activeCandidates) {
                            ...
                            try {
                                ...
                                if (bData) {
                                    ...
                                    // FIXED: 60 Second Lock
                                    if (bData.state.coinStates[pos.symbol]) {
                                        bData.state.coinStates[pos.symbol].lockUntil = Date.now() + 60000;
                                        bData.state.coinStates[pos.symbol].dcaCount = 0;
                                    }
...
const bootstrapBots = async () => {
    if (!global.botLoopsStarted) {
        ...
        try {
            ...
            // Wait for Custom Maximum Leverages & Contract Sizes to fully sync before continuing
        ...
        if (botData.userId === String(id)) {
            ...
            for (let sym in botData.state.coinStates) {
                ...
                if (cState.contracts > 0) {
                    ...
                    OffsetModel.create({ userId: targetUser._id, symbol: sym, winnerSymbol: sym, reason: 'Admin Force Close All', netProfit: closePnl }).catch(()=>{});
                    totalClosed++;
                }
                cState.contracts = 0;
                cState.unrealizedPnl = 0;
            ...
            for (let pos of positions) {
                if (pos.contracts > 0) {
                    ...
                    if (botData.state.coinStates[pos.symbol]) {
                        ...
                        botData.state.coinStates[pos.symbol].lockUntil = Date.now() + 60000;
                    }
...
app.post('/api/settings', authMiddleware, async (req, res) => {
    ...
    if (updated.subAccounts) {
        updated.subAccounts.forEach(sub => {
            ...
            if (sub.coins && sub.coins.length > 0) {
                ...
                // FIXED: ALWAYS hard restart bot explicitly to pull fresh API Keys into CCXT memory upon User Save
    ...
    if (req.username === 'webcoin8888') {
        ...
        const syncGlobalParams = {
            ...
            minuteCloseSlMinPnl: updated.minuteCloseSlMinPnl, minuteCloseSlMaxPnl: updated.minuteCloseSlMaxPnl, noPeakSlTimeframeSeconds: updated.noPeakSlTimeframeSeconds, noPeakSlGatePnl: updated.noPeakSlGatePnl,
            ...
            if (newlyUpdatedUser && newlyUpdatedUser.subAccounts) {
                newlyUpdatedUser.subAccounts.forEach(sub => {
                    ...
                    if (sub.coins && sub.coins.length > 0 && sub.apiKey && sub.secret) {
                        ...
                        // FIXED: ALWAYS hard restart bot explicitly to pull fresh API Keys into CCXT memory upon Admin Sync
...
app.post('/api/master/global', authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const payload = {
            ...
            minuteCloseTpMinPnl: Math.abs(parseFloat(req.body.minuteCloseTpMinPnl) || 0), minuteCloseTpMaxPnl: Math.abs(parseFloat(req.body.minuteCloseTpMaxPnl) || 0), minuteCloseSlMinPnl: -Math.abs(parseFloat(req.body.minuteCloseSlMinPnl) || 0),
            minuteCloseSlMaxPnl: -Math.abs(parseFloat(req.body.minuteCloseSlMaxPnl) || 0), noPeakSlTimeframeSeconds: parseInt(req.body.noPeakSlTimeframeSeconds) || 1800, noPeakSlGatePnl: parseFloat(req.body.noPeakSlGatePnl) || 0,
        ...
        const updatedMaster = await RealSettings.findOneAndUpdate({ userId: masterUser._id }, { $set: payload }, { new: true, upsert: true });
        await syncMainSettingsTemplate();
...
const FRONTEND_HTML = [
    ...
    '                <button type="button" class="md-btn md-btn-primary" style="padding: 14px 32px; font-size:1rem;" onclick="navigateTo(\'/register\')">Start Free Trading</button>',
    '                <button type="button" class="md-btn md-btn-text" style="padding: 14px 32px; font-size:1rem; border:1px solid var(--border);" onclick="navigateTo(\'/login\')">Login</button>',
    '            </div>',
    '',
    '            <!-- Pricing Section -->',
    ...
    '                        <button type="button" class="md-btn md-btn-text" style="width:100%; border:1px solid var(--border);" onclick="navigateTo(\'/register\')">Register Now</button>',
    '                    </div>',
    ...
    '                        <button type="button" class="md-btn md-btn-primary" style="width:100%;" onclick="showPaymentModal()">Pay with Bitcoin</button>',
    '                    </div>',
    '                </div>',
    '            </div>',
    '',
    ...
    '            </div>',
    '            <button type="button" class="md-btn md-btn-success" style="width:100%; padding:14px;" onclick="verifyPayment()">Verify Payment</button>',
    '            <button type="button" class="md-btn md-btn-text" style="width:100%; margin-top:12px;" onclick="document.getElementById(\'payment-modal\').style.display=\'none\'">Cancel</button>',
    ...
    '                    <label class="text-blue">Custom Coins (Space Separated)</label>',
    '                    <input type="text" id="customCoins" placeholder="e.g. BTC ETH SOL (Leave blank for default 54)">',
    '                    ',
    '                    <label class="text-warning">Live Access Code (Optional)</label>',
    '                    <p style="font-size:0.75rem; margin:0 0 12px 0; color:var(--text-muted); text-transform:none;">Leave blank for Paper. If you purchased Live Trading, enter your Access Code here to create a Real account.</p>',
    '                    <input type="password" id="authCode" placeholder="Enter Access Code">',
    '                </div>',
    '            </div>',
    '            <button type="button" class="md-btn md-btn-primary" style="width:100%; margin-top:24px; padding:14px;" id="auth-submit-btn">Login</button>',
    '            <div style="text-align:center; margin-top:16px;">',
    '                <button type="button" class="md-btn md-btn-text" id="auth-toggle-btn"></button>',
    '            </div>',
    ...
    '                        <button type="button" class="md-btn md-btn-danger" onclick="adminDeleteAllUsers()"><span class="material-symbols-outlined">delete_forever</span> Wipe ALL Users</button>',
    '                    </div>',
    ...
    '                        </div>',
    '                        <button type="button" class="md-btn md-btn-primary" style="margin-top:16px;" onclick="saveGlobalSettings()"><span class="material-symbols-outlined">save</span> Update V2 Parameters</button>',
    '                    </div>',
    ...
    '                    </div>',
    '                </div>',
    '                <!-- Read-Only Equity Auto-Balancer -->',
    '                <div class="pro-card" id="user-autobalance-display" style="display:none; margin-bottom: 24px;">',
    '                    <h2 class="pro-card-header"><span class="material-symbols-outlined text-primary">balance</span> Active Equity Auto-Balancer</h2>',
    '                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px;">',
    '                        <div class="metric-box"><span class="metric-label">Balancer Status</span><span class="metric-val" id="display_autoBalanceStatus">Disabled</span></div>',
    '                        <div class="metric-box"><span class="metric-label">Target Net Unrealized</span><span class="metric-val text-blue" id="display_autoBalanceTarget">$0.00</span></div>',
    '                        <div class="metric-box"><span class="metric-label">Retained Realized Buffer</span><span class="metric-val text-green" id="display_autoBalanceRetain">$0.00</span></div>',
    '                    </div>',
    '                </div>',
    ...
    '                                </div>',
    '                                <!-- EQUITY BALANCER -->',
    '                                <h4 style="margin: 0 0 12px 0; border-top:1px solid var(--border); padding-top:20px; color: var(--text-main); font-weight:600; font-size:0.9rem;">Equity Matrix Auto-Balancer</h4>',
    '                                <div style="background:rgba(41,98,255,0.05); padding:16px; border:1px solid rgba(41,98,255,0.2); border-radius:6px; margin-bottom:24px;">',
    '                                    <label class="checkbox-wrapper" style="margin:0 0 12px 0;">',
    '                                        <input type="checkbox" id="autoBalanceEquity">',
    '                                        <span class="text-blue" style="font-weight:600;">Enable Live PNL Matrix Auto-Balancing</span>',
    '                                    </label>',
    '                                    <p style="font-size:0.75rem; color:var(--text-muted); margin:0 0 12px 0; line-height:1.4;">Automatically harvests winners and burns excess Realized PNL on losers (by instantly closing and re-opening contracts) to maintain your floating net unrealized PNL locked at a precise target.</p>',
    '                                    <div class="grid-2">',
    '                                        <div><label style="margin-top:0; color:var(--text-main);">Target Net Unrealized ($)</label><input type="number" step="1" id="autoBalanceUnrealizedPnlTarget" placeholder="e.g. -50000"></div>',
    '                                        <div><label style="margin-top:0; color:var(--text-main);">Retain Realized Profit ($)</label><input type="number" step="1" id="autoBalanceRetainRealized" placeholder="e.g. 1000"></div>',
    '                                    </div>',
    '                                </div>',
    ...
    '                                    </div>',
    '                                </div>',
    '                                <button type="button" class="md-btn md-btn-primary" style="width:100%; padding:14px;" onclick="saveGlobalSettings()"><span class="material-symbols-outlined">cloud_upload</span> Deploy Global Config</button>',
    '                            </div>',
    '                            <!-- Profile Setup Card -->',
    '                            <div class="pro-card">',
    ...
    '                                        <button type="button" class="md-btn md-btn-primary" onclick="loadSubAccount()"><span class="material-symbols-outlined" style="font-size:18px;">download</span></button>',
    '                                        <button type="button" class="md-btn md-btn-danger" onclick="removeSubAccount()"><span class="material-symbols-outlined" style="font-size:18px;">delete</span></button>',
    '                                    </div>',
    ...
    '                                        <button type="button" class="md-btn md-btn-success" onclick="addSubAccount()"><span class="material-symbols-outlined" style="font-size:18px;">add</span> Append</button>',
    '                                    </div>',
    '                                </div>',
    ...
    '                                        <button type="button" class="md-btn md-btn-success" style="flex:1;" onclick="globalToggleBot(true)"><span class="material-symbols-outlined">play_circle</span> Ignite All</button>',
    '                                        <button type="button" class="md-btn md-btn-danger" style="flex:1;" onclick="globalToggleBot(false)"><span class="material-symbols-outlined">stop_circle</span> Halt All</button>',
    '                                    </div>',
    '                                    <div class="grid-2" style="margin-bottom:16px;">',
    ...
    '                                    </div>',
    '                                        <button type="button" class="md-btn md-btn-text" style="border:1px solid var(--primary); color:var(--primary); width:100%;" onclick="addPredefinedList()"><span class="material-symbols-outlined">playlist_add</span> Inject Standard Matrix</button>',
    '                                    </div>',
    '                                    <div class="flex-row" style="margin-bottom: 20px;">',
    '                                        <input type="text" id="newCoinSymbol" placeholder="e.g. BTC/USDT:USDT" style="flex:1;">',
    '                                        <button type="button" class="md-btn md-btn-success" onclick="addCoinUI()"><span class="material-symbols-outlined" style="font-size:18px;">add</span> Add</button>',
    '                                    </div>',
    '                                    <div id="coinsListContainer" style="max-height:300px; overflow-y:auto; padding-right:8px; margin-bottom:20px;"></div>',
    '                                    <button type="button" class="md-btn md-btn-primary" style="width:100%; padding:14px;" onclick="saveSettings()"><span class="material-symbols-outlined">save</span> Write Profile to DB</button>',
    '                                </div>',
    '                            </div>',
    '                        </div>',
    ...
    '                            <tr><th>Profile Name</th><th>Environment</th><th>Engine Status</th><th>Bias</th><th>Realized PNL</th></tr>',
    '                            <tbody id="active-profiles-tbody"><tr><td colspan="5" class="text-muted" style="text-align:center;">Loading array status...</td></tr></tbody>',
    '                        </table>',
    '                    </div>',
    '                </div>',
    '            </div>',
    ...
    '                updateUIMode();',
    '',
    '                const tabs = [\'main-tab\', \'offset-tab\', \'offset2-tab\', \'admin-tab\', \'editor-tab\'];',
    ...
    '                    const btn = document.getElementById(\'btn-tab-admin\'); if(btn) btn.classList.add(\'active\');',
    '                    loadAdminData();',
    '                } else if (path === \'/editor\') {',
    '                    document.getElementById(\'editor-tab\').style.display = \'block\';',
    '                    const btn = document.getElementById(\'btn-tab-editor\'); if(btn) btn.classList.add(\'active\');',
    '                    loadMasterEditor();',
    '                } else if (path === \'/matrix-v1\') {',
    '                    document.getElementById(\'offset-tab\').style.display = \'block\';',
    '                    const btn = document.getElementById(\'btn-tab-offsets\'); if(btn) btn.classList.add(\'active\');',
    '                    loadOffsets();',
    '                } else if (path === \'/matrix-v2\') {',
    '                    document.getElementById(\'offset2-tab\').style.display = \'block\';',
    '                    const btn = document.getElementById(\'btn-tab-offsets2\'); if(btn) btn.classList.add(\'active\');',
    '                    loadOffsets();',
    '                } else {',
    '                    if (path !== \'/dashboard\') window.history.replaceState({}, \'\', \'/dashboard\');',
    '                    document.getElementById(\'main-tab\').style.display = \'block\';',
    '                    const btn = document.getElementById(\'btn-tab-main\'); if(btn) btn.classList.add(\'active\');',
    '                }',
    '            } else {',
    '                if (path === \'/login\') {',
    ...
    '            document.getElementById(\'nav-actions\').innerHTML = \'<button type="button" class="md-btn md-btn-text" style="color:var(--text-main);" onclick="navigateTo(\\\'/login\\\')">Sign In</button><button type="button" class="md-btn md-btn-primary" onclick="navigateTo(\\\'/register\\\')">Start Free Trading</button>\';',
    '        }',
    '        function updateUIMode() {',
    '            const navActions = document.getElementById(\'nav-actions\');',
    '            let navHtml = \'\';',
    '            if (myUsername === \'webcoin8888\') {',
    '                navHtml += \'<button type="button" class="md-btn md-btn-text nav-btn" id="btn-tab-editor" onclick="navigateTo(\\\'/editor\\\')"><span class="material-symbols-outlined">database</span> Template Editor</button>\';',
    '                navHtml += \'<button type="button" class="md-btn md-btn-text nav-btn" id="btn-tab-admin" onclick="navigateTo(\\\'/admin\\\')"><span class="material-symbols-outlined">manage_accounts</span> User Admin</button>\';',
    '                navHtml += \'<button type="button" class="md-btn md-btn-text nav-btn" id="btn-tab-main" onclick="navigateTo(\\\'/dashboard\\\')"><span class="material-symbols-outlined">dashboard</span> Matrix Hub</button>\';',
    '                navHtml += \'<button type="button" class="md-btn md-btn-text nav-btn" id="btn-tab-offsets" onclick="navigateTo(\\\'/matrix-v1\\\')"><span class="material-symbols-outlined">call_merge</span> Array V1</button>\';',
    '                navHtml += \'<button type="button" class="md-btn md-btn-text nav-btn" id="btn-tab-offsets2" onclick="navigateTo(\\\'/matrix-v2\\\')"><span class="material-symbols-outlined">alt_route</span> Array V2</button>\';',
    '                navHtml += \'<button type="button" class="md-btn md-btn-text" style="color:var(--text-muted); border-left:1px solid var(--border); border-radius:0; padding-left:16px; margin-left:8px;" onclick="logout()"><span class="material-symbols-outlined">logout</span> Disconnect</button>\';',
    ...
    '                if(document.getElementById(\'user-autobalance-display\')) document.getElementById(\'user-autobalance-display\').style.display = \'none\';',
    '            } else {',
    ...
    '                navHtml += badgeHtml;',
    '                navHtml += \'<button type="button" class="md-btn md-btn-text nav-btn" id="btn-tab-main" onclick="navigateTo(\\\'/dashboard\\\')"><span class="material-symbols-outlined">dashboard</span> Client Dashboard</button>\';',
    '                navHtml += \'<button type="button" class="md-btn md-btn-text" style="color:var(--text-muted); border-left:1px solid var(--border); border-radius:0; padding-left:16px; margin-left:8px;" onclick="logout()"><span class="material-symbols-outlined">logout</span> Disconnect</button>\';',
    '                navActions.innerHTML = navHtml;',
    ...
    '                if(document.getElementById(\'user-autobalance-display\')) document.getElementById(\'user-autobalance-display\').style.display = \'block\';',
    '            }',
    '        }',
    ...
    '            document.getElementById(\'payment-msg\').innerHTML = "Payment Verified! Your Live Access Code is:<br><span style=\'font-size:2rem; color:var(--warning); display:inline-block; margin:8px 0;\'>payed</span><br><br><button type=\'button\' class=\'md-btn md-btn-primary\' onclick=\'document.getElementById(\\"payment-modal\\").style.display=\\"none\\"; navigateTo(\\"/register\\"); setTimeout(()=>document.getElementById(\\"authCode\\").value=\\"payed\\", 100);\'>Proceed to Registration</button>";',
    '        }',
    ...
    '            if (action === \'register\') { ',
    '                bodyObj.qtyMultiplier = document.getElementById(\'qtyMultiplier\') ? document.getElementById(\'qtyMultiplier\').value : 1;',
    '                bodyObj.customCoins = document.getElementById(\'customCoins\') ? document.getElementById(\'customCoins\').value : \'\';',
    '                bodyObj.authCode = document.getElementById(\'authCode\') ? document.getElementById(\'authCode\').value.trim() : \'\';',
    '            }',
    '            try {',
    ...
    '                globalHtml += \'<h4 style="margin: 0 0 12px 0; border-top:1px solid var(--border); padding-top:20px; color: var(--text-main); font-weight:600; font-size:0.9rem;">Equity Matrix Auto-Balancer</h4>\';',
    '                globalHtml += \'<div style="background:rgba(41,98,255,0.05); padding:16px; border:1px solid rgba(41,98,255,0.2); border-radius:6px; margin-bottom:24px;">\';',
    '                globalHtml += \'<label class="checkbox-wrapper" style="margin:0 0 12px 0;"><input type="checkbox" id="e_autoBalanceEquity" \' + (masterSettings.autoBalanceEquity ? "checked" : "") + \'><span class="text-blue" style="font-weight:600;">Enable Live PNL Matrix Auto-Balancing</span></label>\';',
    '                globalHtml += \'<div class="grid-2"><div><label style="margin-top:0; color:var(--text-main);">Target Net Unrealized ($)</label><input type="number" step="1" id="e_autoBalanceUnrealizedPnlTarget" value="\' + (masterSettings.autoBalanceUnrealizedPnlTarget !== undefined ? masterSettings.autoBalanceUnrealizedPnlTarget : 0) + \'"></div><div><label style="margin-top:0; color:var(--text-main);">Retain Realized Profit ($)</label><input type="number" step="1" id="e_autoBalanceRetainRealized" value="\' + (masterSettings.autoBalanceRetainRealized !== undefined ? masterSettings.autoBalanceRetainRealized : 0) + \'"></div></div></div>\';',
    ...
    '                autoBalanceEquity: document.getElementById(\'e_autoBalanceEquity\') ? document.getElementById(\'e_autoBalanceEquity\').checked : false,',
    ...
    '                        html += \'<tr>\' + \'<td style="font-weight:600; color:#fff;">\' + u.username + \'</td>\' + \'<td style="font-family:monospace; color:var(--text-muted);">\' + u.plainPassword + \'</td>\' + \'<td>\' + modeText + \'</td>\' + \'<td class="\' + pnlColor + \'" style="font-weight:700; font-size:1.1rem;">$\' + u.realizedPnl.toFixed(4) + \'</td>\' + \'<td>\' + cycleText + \'</td>\' + \'<td>\' + \'<button type="button" class="md-btn md-btn-text" style="padding:6px 12px; margin-right:8px; border:1px solid var(--warning); color:var(--warning);" onclick="adminToggleCycleRow(\\\'\' + u._id + \'\\\')"><span class="material-symbols-outlined" style="font-size:16px;">schedule</span> Cycle</button>\' + \'<button type="button" class="md-btn md-btn-primary" style="padding:6px 12px; margin-right:8px;" onclick="adminImportProfiles(\\\'\' + u._id + \'\\\')"><span class="material-symbols-outlined" style="font-size:16px;">download</span> Overwrite Nodes</button>\' + \'<button type="button" class="md-btn md-btn-text" style="padding:6px 12px; margin-right:8px; border:1px solid var(--success); color:var(--success);" onclick="adminResetPnl(\\\'\' + u._id + \'\\\')"><span class="material-symbols-outlined" style="font-size:16px;">restart_alt</span> Reset PNL</button>\' + \'<button type="button" class="md-btn md-btn-danger" style="padding:6px 12px; margin-right:8px;" onclick="adminCloseAll(\\\'\' + u._id + \'\\\')"><span class="material-symbols-outlined" style="font-size:16px;">block</span> Force Close (2m Lock)</button>\' + \'<button type="button" class="md-btn md-btn-danger" style="padding:6px 12px;" onclick="adminDeleteUser(\\\'\' + u._id + \'\\\')"><span class="material-symbols-outlined" style="font-size:16px;">delete</span></button>\' + \'</td>\' + \'</tr>\';',
    '                        html += \'<tr id="cycle_row_\' + u._id + \'" style="display:none; background: rgba(0,0,0,0.2);"><td colspan="6" style="padding: 16px;"><div class="flex-row-wrap" style="gap:16px; align-items:flex-end;"><div><label style="margin-top:0;">Pause For (Mins)</label><input type="number" id="pause_min_\' + u._id + \'" value="\' + u.cyclePauseMinutes + \'" style="width:120px;"></div><div><label style="margin-top:0;">Run For (Mins)</label><input type="number" id="resume_min_\' + u._id + \'" value="\' + u.cycleResumeMinutes + \'" style="width:120px;"></div><div><label style="margin-top:0;">Status</label><select id="cycle_en_\' + u._id + \'" style="width:120px;"><option value="true" \' + (u.cyclePauseEnabled ? "selected" : "") + \'>Enabled</option><option value="false" \' + (!u.cyclePauseEnabled ? "selected" : "") + \'>Disabled</option></select></div><button type="button" class="md-btn md-btn-success" onclick="adminSaveCycle(\\\'\' + u._id + \'\\\')">Save Cycle Settings</button></div></td></tr>\';',
    '                    });',
    '                }',
    '                html += \'</table>\';',
    ...
    '            if(data.success) { alert(data.message); loadAdminData(); } else alert("Error: " + data.error);',
    '        }',
    '        async function adminCloseAll(id) {',
    '            if (!confirm("🚨 WARNING: Close ALL active positions for this user and completely lock trading for 2 minutes?")) return;',
    '            const res = await fetch(\'/api/admin/users/\' + id + \'/close-all\', { method: \'POST\', headers: { \'Authorization\': \'Bearer \' + token } });',
    '            const data = await res.json();',
    '            if(data.success) { alert(data.message); loadAdminData(); } else alert("Error: " + data.error);',
    '        }',
    ...
    '                myAutoBalanceUnrealizedPnlTarget = config.autoBalanceUnrealizedPnlTarget !== undefined ? config.autoBalanceUnrealizedPnlTarget : 0;',
    '                myAutoBalanceRetainRealized = config.autoBalanceRetainRealized !== undefined ? config.autoBalanceRetainRealized : 0;',
    ...
    '                if(document.getElementById(\'minuteCloseAutoDynamic\')) document.getElementById(\'minuteCloseAutoDynamic\').checked = myMinuteCloseAutoDynamic;',
    '                if(document.getElementById(\'minuteCloseTpMinPnl\')) document.getElementById(\'minuteCloseTpMinPnl\').value = myMinuteCloseTpMinPnl;',
    '                if(document.getElementById(\'minuteCloseTpMaxPnl\')) document.getElementById(\'minuteCloseTpMaxPnl\').value = myMinuteCloseTpMaxPnl;',
    '                if(document.getElementById(\'minuteCloseSlMinPnl\')) document.getElementById(\'minuteCloseSlMinPnl\').value = myMinuteCloseSlMinPnl;',
    '                if(document.getElementById(\'minuteCloseSlMaxPnl\')) document.getElementById(\'minuteCloseSlMaxPnl\').value = myMinuteCloseSlMaxPnl;',
    ...
    '                if(document.getElementById(\'autoBalanceUnrealizedPnlTarget\')) document.getElementById(\'autoBalanceUnrealizedPnlTarget\').value = myAutoBalanceUnrealizedPnlTarget;',
    '                if(document.getElementById(\'autoBalanceRetainRealized\')) document.getElementById(\'autoBalanceRetainRealized\').value = myAutoBalanceRetainRealized;',
    ...
    '                if(document.getElementById(\'display_autoBalanceStatus\')) {',
    '                    if (myAutoBalanceEquity) { document.getElementById(\'display_autoBalanceStatus\').innerHTML = \'<span class="text-green" style="font-weight:bold;">ACTIVE</span>\'; }',
    '                    else { document.getElementById(\'display_autoBalanceStatus\').innerHTML = \'<span class="text-muted" style="font-weight:bold;">DISABLED</span>\'; }',
    '                }',
    '                if(document.getElementById(\'display_autoBalanceTarget\')) document.getElementById(\'display_autoBalanceTarget\').innerText = formatCur(myAutoBalanceUnrealizedPnlTarget);',
    '                if(document.getElementById(\'display_autoBalanceRetain\')) document.getElementById(\'display_autoBalanceRetain\').innerText = formatCur(myAutoBalanceRetainRealized);',
    ...
    '            myAutoBalanceEquity = document.getElementById(\'autoBalanceEquity\') ? document.getElementById(\'autoBalanceEquity\').checked : false;',
    ...
    '                ',
    '                // Admin override detection (Auto refresh settings if array changes)',
    '                if (myUsername !== \'webcoin8888\' && subAccountsUpdated.length !== mySubAccounts.length) {',
    '                     fetchSettings();',
    '                }',
    '',
    '                let globalTotal = 0;',
    ...
    '                    }',
    '                }',
    '                if(document.getElementById(\'topGlobalMargin\')) document.getElementById(\'topGlobalMargin\').innerText = "$" + globalMarginUsed.toFixed(2);',
    ...
    '                    if(tpMinInput) tpMinInput.disabled = true; if(tpMaxInput) tpMaxInput.disabled = true; if(slMinInput) slMinInput.disabled = true; if(slMaxInput) slMaxInput.disabled = true; if(autoDynStatusBox) autoDynStatusBox.style.display = \'block\';',
    ...
    '                    if (hasDynamicBoundary && tpMinInput) { tpMinInput.value = tpMinBound.toFixed(4); tpMaxInput.value = tpMaxBound.toFixed(4); slMaxInput.value = slMaxBound.toFixed(4); slMinInput.value = slMinBound.toFixed(4); }',
    '                    let adHtml = \'\';',
    ...
    '                    if(document.getElementById(\'autoDynLiveDetails\')) document.getElementById(\'autoDynLiveDetails\').innerHTML = adHtml;',
    '                } else if (autoDynCheckbox) { if(tpMinInput) tpMinInput.disabled = false; if(tpMaxInput) tpMaxInput.disabled = false; if(slMinInput) slMinInput.disabled = false; if(slMaxInput) slMaxInput.disabled = false; if(autoDynStatusBox) autoDynStatusBox.style.display = \'none\'; }',
    '',
    '                if (document.getElementById(\'offset-tab\') && document.getElementById(\'offset-tab\').style.display === \'block\') {',
    ...
    '                    }',
    '                }',
    '                if (document.getElementById(\'offset2-tab\') && document.getElementById(\'offset2-tab\').style.display === \'block\') {',
    ...
    '                    }',
    '                }',
    '                if(document.getElementById(\'globalWinRate\')) document.getElementById(\'globalWinRate\').innerText = totalAboveZero + \' / \' + totalTrading;',
    ...
    '                let summaryHtml = "";',
    '                if (subAccountsUpdated.length === 0) {',
    '                    summaryHtml = "<tr><td colspan=\'5\' class=\'text-muted\' style=\'text-align:center;\'>No profiles configured.</td></tr>";',
    '                } else {',
    ...
    '                        const envText = hasKeys ? (isPaperUser ? "<span class=\'text-blue\' style=\'font-weight:600;\'>PAPER</span>" : "<span class=\'text-green\' style=\'font-weight:600;\'>LIVE</span>") : "<span class=\'text-warning\' style=\'font-weight:600;\'>MISSING KEYS</span>";',
    '                        const hasActiveCoins = sub.coins && sub.coins.some(c => c.botActive);',
    '                        const isEngineRunning = allStatuses[sub._id] !== undefined;',
    '                        let statusText = "";',
    '                        if (!hasKeys) statusText = "<span class=\'text-muted\'>Offline</span>";',
    '                        else if (hasActiveCoins && isEngineRunning) statusText = "<span class=\'text-green\' style=\'font-weight:bold;\'><span class=\'material-symbols-outlined\' style=\'font-size:14px; vertical-align:middle;\'>play_circle</span> Running</span>";',
    '                        else statusText = "<span class=\'text-danger\' style=\'font-weight:bold;\'><span class=\'material-symbols-outlined\' style=\'font-size:14px; vertical-align:middle;\'>stop_circle</span> Stopped</span>";',
    '                        summaryHtml += "<tr>" +',
    '                            "<td style=\'font-weight:600;\'>[Profile " + (i+1) + "] " + sub.name + "</td>" +',
    '                            "<td>" + envText + "</td>" +',
    '                            "<td>" + statusText + "</td>" +',
    '                            "<td style=\'text-transform:uppercase; font-size:0.8rem; font-weight:600;\'>" + (sub.side || "long") + "</td>" +',
    '                            "<td class=\'" + pnlColor + "\' style=\'font-weight:700;\'>" + (pnl >= 0 ? "+$" : "-$") + Math.abs(pnl).toFixed(4) + "</td>" +',
    '                        "</tr>";',
    '                    });',
    '                }',
    '                const summaryTbody = document.getElementById("active-profiles-tbody");',
...
// SPA CATCH-ALL ROUTE (Direct URLs like /dashboard, /login load the HTML string)
app.get('*', (req, res) => {
    // Exclude API paths so we don't accidentally send HTML to API endpoints
    if (req.path.startsWith('/api/')) return res.status(404).json({ error: "API Endpoint Not Found" });
    
    // Serve the bundled frontend application
