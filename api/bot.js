...
app.get('/', (req, res) => {
    res.send([
        ...
        '                            else if (isSlHit) { let blockedByLimit = (maxLossPerMin > 0 && (currentMinuteLoss + Math.abs(net)) > maxLossPerMin); if (blockedByLimit) { statusIcon = \'block Rate Limited\'; if (topStatusMessage2.includes(\'Scanning\')) topStatusMessage2 = \'<span class="text-danger" style="font-weight:bold;">🔴 V2 Stop Loss Blocked by Limit Rate.</span>\'; } else { statusIcon = \'block Liquidating\'; topStatusMessage2 = \
        '                            <span class="text-danger" style="font-weight:bold;">🔴 Engaging Array Node \' + (winnerIndex+1) + \' for Stop Loss!</span>\'; } }',
