...
function getLeverageForCoin(symbol) { if (global.customMaxLeverages && global.customMaxLeverages[symbol]) return global.customMaxLeverages[symbol]; if (symbol.includes('BTC')) return 125; if (symbol.includes('ETH')) return 100; return 20; }

// ==========================================
// GLOBAL PRICE ORACLE (BINANCE + HTX)
// ==========================================
global.livePrices = global.livePrices || {}; let isBinanceFetching = false; let isHtxFetching = false;
...
function startPriceOracle() {
    console.log("🔮 Starting Global Price Oracle...");
    setInterval(async () => { if (isBinanceFetching) return; isBinanceFetching = true; try { const tickers = await binanceOracle.fetchTickers(); for (let sym in tickers) { if (tickers[sym] && tickers[sym].last) global.livePrices[sym] = tickers[sym].last; } } catch(e) { } finally { isBinanceFetching = false; } }, 3000);
    setInterval(async () => { if (isHtxFetching) return; isHtxFetching = true; try { const tickers = await htxOracle.fetchTickers(); for (let sym in tickers) { if (tickers[sym] && tickers[sym].last) global.livePrices[sym] = tickers[sym].last; } } catch(e) { } finally { isHtxFetching = false; } }, 5000);
...
// ==========================================
global.activeBots = global.activeBots || new Map(); const activeBots = global.activeBots;

function logForProfile(profileId, msg) {
    console.log(`[Profile: ${profileId}] ${msg}`); const bot = activeBots.get(profileId);
        ...
        try {
            ...
            for (let coin of activeCoins) {
                try {
                    ...
                    if (cState.contracts <= 0) {
                        const safeBaseQty = Math.max(1, Math.floor(currentSettings.baseQty)); logForProfile(profileId, `[${isPaper ? "PAPER" : "REAL"}] 🛒 Opening base position of ${safeBaseQty} contracts (${activeSide}) at ~${cState.currentPrice} using ${activeLeverage}x Leverage.`);
                        ...
                        const OffsetModel = isPaper ? PaperOffsetRecord : RealOffsetRecord; OffsetModel.create({ userId: userId, symbol: coin.symbol, winnerSymbol: coin.symbol, reason: `Open Base Position (${safeBaseQty} contracts)`, netProfit: 0 }).catch(()=>{});
                        cState.lockUntil = Date.now() + 5000; continue; 
                    }

                    // 2. TAKE PROFIT OR STOP LOSS
                    let isTakeProfit = false; let tpReasonTxt = '';
                    let isStopLoss = false; let slReasonTxt = '';
                    ...
                    const globalSlPnlTarget = parseFloat(botData.globalSettings?.globalSingleCoinSlPnl) || 0; // NEW INDIVIDUAL GLOBAL SL
                    ...
                    if (baseTriggerPnl < 0 && cState.unrealizedPnl <= activeTriggerPnl && (Date.now() - (cState.lastDcaTime || 0) > 12000)) {
                        ...
                        if (reqQty <= 0) cState.lastDcaTime = Date.now();
                            ...
                            logForProfile(profileId, `[${isPaper ? 'PAPER' : 'REAL'}] ⚡ DCA Step ${currentDcaStep + 1}: Buying ${reqQty} contracts (Targeting $${targetPnlForDca.toFixed(2)} recovery) at ${activeLeverage}x. Next scaled target: $${nextTarget.toFixed(2)}.`);
                            ...
                            const OffsetModel = isPaper ? PaperOffsetRecord : RealOffsetRecord; OffsetModel.create({ userId: userId, symbol: coin.symbol, winnerSymbol: coin.symbol, reason: `DCA Step ${currentDcaStep + 1}: Added ${reqQty} Contracts`, netProfit: 0 }).catch(()=>{});
        ...
        for (let userSetting of allUsersSettings) {
            ...
            for (let i = 0; i < totalPairs; i++) {
                ...
                if (!executedGroup && (isPositiveMatch || isNegativeMatch)) {
                    ...
                    if (successCount > 0) {
                        await SettingsModel.updateOne({ userId: dbUserId }, { $set: { autoDynamicLastExecution: { time: Date.now(), type: executionType, symbol: `Group up to Row ${i + 1} (WINNERS)`, pnl: runningAccumulation } } }).catch(()=>{});
                        const OffsetModel = userSetting.isPaper ? PaperOffsetRecord : RealOffsetRecord; OffsetModel.create({ userId: dbUserId, symbol: `Group of ${i + 1} Coins`, winnerSymbol: `Group of ${i + 1} Coins`, reason: `1-Min Closer Executed (${executionType})`, netProfit: runningAccumulation }).catch(()=>{});
        ...
        for (let userSetting of allUsersSettings) {
            ...
            if ((smartOffsetNetProfit > 0 || smartOffsetBottomRowV1StopLoss < 0 || smartOffsetStopLoss < 0 || noPeakMs > 0) && activeCandidates.length >= 2) {
                ...
                if (triggerOffset) {
                    ...
                    if (triggerOffset) {
                        logForProfile(firstProfileId, `⚖️ SMART OFFSET V1 [${reason}]: Closing ${finalPairsToClose.length} coin(s).`); let successCount = 0;
                    ...
                    for (let pos of activeCandidates) {
                        ...
                        try {
                            ...
                            if (bData) {
                                ...
                                pos.subAccount.realizedPnl = (pos.subAccount.realizedPnl || 0) + pos.unrealizedPnl; await SettingsModel.updateOne({ "subAccounts._id": pos.subAccount._id }, { $set: { "subAccounts.$.realizedPnl": pos.subAccount.realizedPnl } }).catch(()=>{});
                            }
                        } catch(e) { logForProfile(firstProfileId, `❌ CLOSE ERROR [${pos.symbol}]: ${e.message}`); }
                    }
...
const authMiddleware = async (req, res, next) => {
    ...
    jwt.verify(token, JWT_SECRET, async (err, decoded) => { if (err) return res.status(403).json({ error: 'Invalid token' }); req.userId = decoded.userId; const user = await User.findById(decoded.userId); if (!user) return res.status(401).json({ error: 'User not found' }); req.isPaper = user.isPaper; req.username = user.username; next(); });
};
        ...
        if (!templateSettings.subAccounts || templateSettings.subAccounts.length === 0) {
            ...
            for (let i = 1; i <= 6; i++) {
                ...
                PREDEFINED_COINS.forEach((base, index) => {
                    ...
                    if (i === 1) { coinSide = (index % 2 === 0) ? 'long' : 'short'; profileName = "P1: Even L / Odd S"; } else if (i === 2) { coinSide = (index % 2 === 0) ? 'short' : 'long'; profileName = "P2: Even S / Odd L"; } else if (i === 3) { coinSide = 'long'; profileName = "P3: All Long"; } else if (i === 4) { coinSide = 'short'; profileName = "P4: All Short"; } else if (i === 5) { coinSide = (index < PREDEFINED_COINS.length / 2) ? 'long' : 'short'; profileName = "P5: Half L / Half S"; } else if (i === 6) { coinSide = (index < PREDEFINED_COINS.length / 2) ? 'short' : 'long'; profileName = "P6: Half S / Half L"; }
...
app.post('/api/admin/users/:id/import', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    if (updatedUser && updatedUser.subAccounts) { updatedUser.subAccounts.forEach(sub => { if (sub.coins && sub.coins.length > 0 && sub.apiKey && sub.secret) { sub.coins.forEach(c => c.botActive = true); startBot(targetUser._id.toString(), sub, targetUser.isPaper).catch(()=>{}); } }); } res.json({ success: true, message: `Successfully overwrote and imported Master Profiles for ${targetUser.username}.` });
});
app.delete('/api/admin/users/:id', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    for (let [profileId, botData] of activeBots.entries()) { if (botData.userId === String(req.params.id)) stopBot(profileId); }
    await User.findByIdAndDelete(req.params.id); await PaperSettings.deleteMany({ userId: req.params.id }); await RealSettings.deleteMany({ userId: req.params.id }); await PaperProfileState.deleteMany({ userId: req.params.id }); await RealProfileState.deleteMany({ userId: req.params.id }); await PaperOffsetRecord.deleteMany({ userId: req.params.id }); await RealOffsetRecord.deleteMany({ userId: req.params.id }); res.json({ success: true, message: 'Deleted user ' + targetUser.username });
});
        ...
        for (let [profileId, botData] of activeBots.entries()) {
            ...
            for (let pos of positions) {
                if (pos.contracts > 0) {
                    ...
                    await botData.exchange.createOrder(pos.symbol, 'market', closeSide, pos.contracts, undefined, { offset: 'close', reduceOnly: true, lever_rate: activeLev }).catch(console.error); totalClosed++; let closedPnl = 0;
...
app.post('/api/settings', authMiddleware, async (req, res) => {
    ...
    if (req.username === 'webcoin8888') {
        ...
        const applyMasterSync = async (userSettingsDoc, isPaperMode) => {
            ...
            updatePayload.smartOffsetNetProfit = (updated.smartOffsetNetProfit || 0) * mult; updatePayload.noPeakSlGatePnl = (updated.noPeakSlGatePnl || 0) * mult; updatePayload.globalSingleCoinTpPnl = (updated.globalSingleCoinTpPnl || 0) * mult; updatePayload.globalSingleCoinSlPnl = (updated.globalSingleCoinSlPnl || 0) * mult; updatePayload.globalTriggerDcaPnl = (updated.globalTriggerDcaPnl || 0) * mult;
            if (!isPaperMode) {
            ...
            const ModelToUse = isPaperMode ? PaperSettings : RealSettings; const newlyUpdatedUser = await ModelToUse.findOneAndUpdate({ userId: userSettingsDoc.userId }, { $set: updatePayload }, { returnDocument: 'after' });
            const userActiveSubIds = [];
            if (newlyUpdatedUser && newlyUpdatedUser.subAccounts) {
                newlyUpdatedUser.subAccounts.forEach(sub => { const profileId = sub._id.toString(); userActiveSubIds.push(profileId); if (sub.coins && sub.coins.length > 0 && sub.apiKey && sub.secret) { sub.coins.forEach(c => c.botActive = true); if (activeBots.has(profileId)) { activeBots.get(profileId).settings = sub; activeBots.get(profileId).globalSettings = newlyUpdatedUser; } else { startBot(newlyUpdatedUser.userId.toString(), sub, isPaperMode).catch(()=>{}); } } else { stopBot(profileId); } });
            }
                ...
                <div class="flex-row" style="align-items: stretch;">
                    <div class="md-card flex-1">
                        ...
                        <div class="stat-box" style="margin-bottom: 24px;">
                            ...
                            <div class="flex-row" style="margin-top:16px;"><div style="flex:1;"><label class="text-green" style="margin-top:0;">Global Single Coin TP ($)</label><input type="number" step="0.0001" id="globalSingleCoinTpPnl"></div><div style="flex:1;"><label class="text-danger" style="margin-top:0;">Global Single Coin SL ($)</label><input type="number" step="0.0001" id="globalSingleCoinSlPnl"></div><div style="flex:1;"><label class="text-danger" style="margin-top:0;">Global Trigger DCA ($)</label><input type="number" step="0.0001" id="globalTriggerDcaPnl"></div></div>
    ...
    <script>
        ...
        async function fetchSettings() {
            try {
                ...
                ['globalTargetPnl','globalTrailingPnl','globalSingleCoinTpPnl','globalSingleCoinSlPnl','globalTriggerDcaPnl','smartOffsetNetProfit','smartOffsetBottomRowV1','smartOffsetBottomRowV1StopLoss','smartOffsetStopLoss','smartOffsetNetProfit2','smartOffsetStopLoss2','smartOffsetMaxLossPerMinute','smartOffsetMaxLossTimeframeSeconds','minuteCloseTpMinPnl','minuteCloseTpMaxPnl','minuteCloseSlMinPnl','minuteCloseSlMaxPnl','noPeakSlTimeframeSeconds','noPeakSlGatePnl'].forEach(k => { window['my'+k.charAt(0).toUpperCase()+k.slice(1)] = c[k]!==undefined?c[k]:(k.includes('Timeframe')?60:(k==='smartOffsetBottomRowV1'?5:(k==='noPeakSlTimeframeSeconds'?1800:0))); document.getElementById(k).value = window['my'+k.charAt(0).toUpperCase()+k.slice(1)]; });
        ...
        async function saveGlobalSettings() {
            const p = ['globalTargetPnl','globalTrailingPnl','globalSingleCoinTpPnl','globalSingleCoinSlPnl','globalTriggerDcaPnl','smartOffsetNetProfit','smartOffsetBottomRowV1StopLoss','smartOffsetStopLoss','smartOffsetNetProfit2','smartOffsetStopLoss2','smartOffsetMaxLossPerMinute','noPeakSlGatePnl'];
