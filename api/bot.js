try { require('dotenv').config(); } catch (e) {} // Safe fallback for Vercel
...
async function executeSmartOffsetV1(userId, isPaper, globalSettings, activePairs) {
    ...
    let slEnabled = true;
    if (stopLossNth < 0 && nthBottomAccumulation > stopLossNth) slEnabled = false;
    ...
    if (slEnabled && fullGroupSl < 0 && runningAccumulation <= fullGroupSl) {
        ...
        if (!canExecuteStopLoss(globalSettings, lossAmount)) return false;
    ...
    for (let i = 0; i < totalPairs; i++) {
        ...
        if (targetV2 > 0 && net >= targetV2) {
            ...
            if (b && b.state.coinStates[w.symbol] && (!b.state.coinStates[w.symbol].lockUntil || Date.now() >= b.state.coinStates[w.symbol].lockUntil)) {
                ...
                const OffsetModel = isPaper ? PaperOffsetRecord : RealOffsetRecord;
                OffsetModel.create({ userId, symbol: w.symbol, winnerSymbol: w.symbol, loserSymbol: l.symbol, reason: `Smart Offset V2 TP`, netProfit: w.pnl }).catch(()=>{});
                closedSomething = true;
            }
        } 
            ...
            if (b && b.state.coinStates[w.symbol] && (!b.state.coinStates[w.symbol].lockUntil || Date.now() >= b.state.coinStates[w.symbol].lockUntil)) {
                ...
                const OffsetModel = isPaper ? PaperOffsetRecord : RealOffsetRecord;
                OffsetModel.create({ userId, symbol: w.symbol, winnerSymbol: w.symbol, loserSymbol: l.symbol, reason: `Smart Offset V2 SL`, netProfit: w.pnl }).catch(()=>{});
                closedSomething = true;
            }
        }
    }
...
function stopBot(profileId) {
    if (activeBots.has(profileId)) activeBots.delete(profileId);
}
...
app.get('/api/me', authMiddleware, (req, res) => { res.json({ userId: req.userId, isPaper: req.isPaper, username: req.username }); });
...
app.get('/api/admin/users', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    for (let u of users) {
        ...
        let realizedPnl = 0;
        if (settings && settings.subAccounts) { settings.subAccounts.forEach(sub => realizedPnl += (sub.realizedPnl || 0)); }
...
app.post('/api/admin/users/:id/import', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    doc.subAccounts = masterSettings.subAccounts.map((masterSub, i) => {
        ...
        return {
            ...
            realizedPnl: existingSub.realizedPnl || 0, coins: masterSub.coins.map(c => ({ symbol: c.symbol, side: c.side, botActive: false }))
        };
    });
        ...
        for (let [profileId, botData] of activeBots.entries()) {
            ...
            for (let pos of positions) {
                if (pos.contracts > 0) {
                    ...
                    await botData.exchange.createOrder(pos.symbol, 'market', closeSide, pos.contracts, undefined, { offset: 'close' }).catch(()=>{});
                    totalClosed++;
                    let closedPnl = 0;
...
app.post('/api/settings', authMiddleware, async (req, res) => {
    ...
    if (updated.subAccounts) {
        updated.subAccounts.forEach(sub => {
            ...
            if (sub.coins && sub.coins.length > 0) {
                ...
                else startBot(req.userId.toString(), sub, req.isPaper).catch(()=>{}); 
    ...
    if (req.username === 'webcoin8888') {
        ...
        const applyMasterSync = async (userSettingsDoc, isPaperMode) => {
            ...
            if (!isPaperMode) {
                updatePayload.subAccounts = updated.subAccounts.map((masterSub, index) => {
...
// ==========================================
// 7. FRONTEND UI (SAFE STRING BUILD)
// ==========================================
app.get('/', (req, res) => {
    ...
    <script>
                        ...
                        profilesHtml += '<div class="stat-box" style="margin-bottom: 24px; border: 1px solid var(--primary); background: #fff;">' +
                            ...
                            '<div style="overflow-x:auto;">' +
                                '<table class="md-table" style="margin-bottom: 16px;">' +
                                    '<tr><th>Base Qty</th><th>Take Profit %</th><th>Stop Loss %</th><th>DCA Trigger %</th><th>Target ROI %</th><th>Max Contracts</th></tr>' +
                                    '<tr>' +
        ...
        async function saveMasterGlobalSettings() {
            ...
            try {
                ...
                const data = await res.json();
                if (data.success) { msgDiv.className = "text-green"; msgDiv.innerText = data.message; } 
                else { msgDiv.className = "text-red"; msgDiv.innerText = "Error: " + data.error; }
        ...
        async function loadAdminData() {
            try {
                ...
                if (statusData.templateSafe && statusData.webcoinSafe) {
                    ...
                    banner.style.background = "#FFEBEE"; banner.style.color = "var(--danger)";
        ...
        function logout() { 
            ...
            mySubAccounts = []; myCoins = []; currentProfileIndex = -1;
        ...
        async function fetchSettings() {
            try {
                ...
                if (mySubAccounts.length > 0) {
                    document.getElementById("subAccountSelect").value = 0; loadSubAccount();
                } else {
                    currentProfileIndex = -1; document.getElementById("settingsContainer").style.display = "none";
                    document.getElementById("dashboardStatusContainer").innerHTML = "<p>No profile loaded.</p>"; myCoins = [];
                }
        ...
        async function loadStatus() {
            ...
            let globalUnrealized = 0; let totalTrading = 0; let totalAboveZero = 0; let globalMarginUsed = 0; 
            ...
            if (autoDynCheckbox && autoDynCheckbox.checked) {
                ...
                if (hasDynamicBoundary && totalPairs > 0) {
                    let highestGroupAcc = -99999; let lowestGroupAcc = 99999;
                    let highestGroupIndex = -1; let lowestGroupIndex = -1;
            ...
            if (document.getElementById("offset2-tab").style.display === "block") {
                ...
                if (stopLossNth < 0 && nthBottomAccumulation > stopLossNth) v2SlEnabled = false;
